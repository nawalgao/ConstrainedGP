...
class ThermalPrefDataGen(object):
    def __init__(self, config_file):
        ...
        self.temp_diff = config['Grid']['temp_diff']
...
class ReachableStates(object):
    ...
    def reachable(self, config_file):
        ...
        with open(config_file, 'r') as fd:
            config = json.loads(fd.read())
        temp_diff = config['Grid']['temp_diff']
        grid_min = config['Grid']['min1']
        grid_max = config['Grid']['max1']
        reach_min = self.Xprev - temp_diff
        reach_max = self.Xprev + temp_diff
        if reach_min < grid_min:
            reach_min = grid_min
        if reach_max > grid_max:
            reach_max = grid_max
        Xr = np.arange(reach_min, reach_max, temp_diff)[:,None]
