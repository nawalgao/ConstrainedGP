...
def train(X, Y, config_file, model_num = 3):
    """
    ...
    if model_num == 1:
        ...
        # Model definition
        m = build_model.GpPrefLearningHMC(Xn, Y, k, l)
    ...
    if model_num == 3:
        ...
        # Model definition
        m = build_model.GpPrefLearningHMC(Xn, Y, k, l, meanf)
        m.kern.lengthscales = config['Hyper_Config']['l']
        m.kern.lengthscales.fixed = True
        m.kern.variance = config['Hyper_Config']['v']
        m.kern.variance.fixed = True
...
if __name__ == '__main__':
    ...
    save_file_name = '../data/initial_duels/train2D.npz'
    data = np.load(save_file_name)
    V = datagen.ThermalPrefDataGen(config_file)
    X = data['X']
    Y = data['Y']
    Xnorm = V.normalize2Dpairwise(X)
   
    m, samples = train(Xnorm, Y, config_file)
