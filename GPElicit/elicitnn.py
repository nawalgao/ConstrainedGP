...
class Aquisition(object):
    def __init__(self, X, Y, config_file, hyperparameters = 'fixed'):
         """
         ...
         self.m, self.samples = train.train(self.Xtrainnorm, Y,
                                            config_file, self.hyperparameters)
    ...
    def sanity_checks(self, iter_num, trial_num, mean_EUI, savefig):
        ...
        if self.hyperparameters == 'fixed':
            ...
            for i in xrange(25,75):
                visualize.visualize_utility(i, iter_num,trial_num, self.Xreachable,
                                            self.mreachablemat[i,:],
                                            self.varreachablemat[i,:])
    ...
    def EUI(self, iter_num, trial_num, savefig = True):
        ...
        np.savez(('../data/results/T' + str(trial_num) +
                  '/exp_imp_saves/' + str(iter_num) + '.npz'), mean_exp_imp = mean_exp_imp)
...
def seq_learning(X, Y, budget, config_file, trial_num, savefig = False):
    """
...
if __name__ == '__main__':
    ...
    data1 = np.load(save_file_name2)
    X1 = data1['X']
    Y1 = data1['Y']
    budget = 5
    trial_num = 2
    
    S1 = seq_learning(X1, Y1, budget, config_file, trial_num, savefig = False)
